...
快照应放在当前数据目录的 `strategy_backtest_snapshots/`，旧单文件入口是 `strategy_backtest_snapshot.json`。APP 会自动读取这些位置，没有任意路径的快照导入向导。正常保存只保留最近 8 份目录快照。新冻结方案在独立的 `strategy_experiments/` 中长期保存，不参与该清理；参数对比报告保存在 `strategy_research_results/`。这两个目录应随个人数据备份。CSV 只是导出结果，不能作为策略快照直接导回。
...
- akshare 的股息率兜底可能取实时快照，未必代表历史估值日。新增观察值按历史/实时/失败分别缓存，结果保存来源和观察日期；已有历史取值持久缓存，实时值最多一小时，失败最多五分钟。需要可重复的情景对比，可在策略 GUI 填固定「股息率%」或 CLI 传 `--q`（小数制）；这不等于历史股息率已知。单债 GUI 回测沿用定价页固定的 `q`。
- 策略收益改为持续持仓的 `cash_ledger_v1` 账本：下一收盘成交前旧仓继续持有，缺价仓位不能按陈旧报价卖出，买卖两边分别计费，合同应收与确认到账分开。净值、收益、换手和现金可能与旧版不同；旧快照保留原结果，不自动重算。只能确定合同票息而不能确认付款的记录仍有总回报与税款边界，见「诊断」与 CSV 资金流水。
- 冻结方案必须来自当前版本实际完成的回测；旧快照缺完整设置时先重跑。未来记录按固定起点完整复评，只有新的完整调仓区间可追加。修改经济计算口径或修订数据导致旧期结果变化时，会要求新版本或报历史冲突，不会覆盖已保存的样本外结果。
