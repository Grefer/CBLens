...
`代码 / 名称 / 正股 / 事件 / 市价 / 涨跌 vs MM-DD / 理论价 / 偏差(%) / 偏差Δ(pp) / 相对偏差 / 双低 / 敏感性 / 标签 / 数据 / 加入时间`

- **涨跌 / 偏差Δ** 锚**上一交易日**的窄快照（`data/watchlist_daily/YYYY-MM-DD.json`）。表头写的是动态日期而不是「日涨跌」——基准由盘上有没有那天的文件决定，周一或长假后它其实是 3 天涨跌，你没开过 GUI 的那些天更是直接跳过去。**没有基准时显示「—」而不是 0.00**。
- **数据** 列把三种长得一样的「—」分开：

  | 显示 | 含义 |
  | :--- | :--- |
  | `✓ 今日` / `昨日` | 有价；估值日是不是今天 |
  | `✓ 今日 · 价 08-21` | 市价本身落在几天前（停牌、节假日） |
  | `✓ 今日 · 无戳` | 走条款库 `close` 兜底，**没有 as-of**，可能任意旧 |
  | `无市价` | 算过了，但数据源没给市价 |
...
1. 选择视图：综合机会 / 低估候选 / 下修优势 / 双低 / 转股折价 / 需复核（菜单里带实时条数）
2. 选择列预设：简洁（13 列）/ 完整（22 列）
3. 点击 **🔄 刷新重算**
4. 主表默认按**当期横截面相对便宜度**排序，查看理论价、市价、相对偏差、双低、下修优势、事件、风险标签
...
| `review_bucket` | 互斥分桶：需复核 / 模型存疑 / 低估候选 / 转股折价 / 综合机会 |
...
（那之后候选口径已改成横截面两道闸，机会分不再当阈值用；当前池机会分最大值只有 6.48，
`≥8` 一只都没有，见 [4.2 批量页](#42--批量页)）。
...
策略页围绕 CBLens 自有定价模型保留两类策略：

- **下修机会**（默认）：反解市场隐含下修强度，与事件模型给定强度比较；在 HV 与信用利差扰动的四个角点中取最差优势排序。
- **估值偏差**：只保留市价低于理论价的标的，按 `(market - theoretical) / theoretical` 从低到高排序。

两类策略都按 Top N 等权持有；候选不足或缺成交价时**保留现金**，不再提供“等权全池/缺口摊回”选项。旧机会分、双低和旧模板只保留在历史快照/Python API 兼容层。
...
3. “稳健下修优势”只代表在当前 HV/利差扰动带下仍为正，不代表样本外收益已显著;
4. 「诊断」下半部分核对条款来源、模型参数、公告修正与缺价跳过数；该页应与快照中的设置一致。

**进阶旋钮 (高级研究)**

- **事件退出**: 下修提议/通过/拒绝公告会让 thesis 落地或证伪；默认在公告后下一可得收盘退出，余下时间持有现金。
