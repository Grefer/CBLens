...
一键完成「同步条款库 → 扫描新债 → 加入关注池 → 立刻定价」：

- 本地条款库超过 1 天没同步时会先问要不要跑一次增量同步（需 Wind 终端）；新债每天都在发，只扫本地快照必然漏。
- 两类都算新债：**已定上市日**（未来 30 天内挂牌）与**已发行未上市**（数据源还没给上市日，表里上市日/可交易日显示「待定」）。
- 扫完自动对关注池里还没有理论价的标的定价，新债一进来就带着理论价，而不是一行空白指标。

> [!NOTE]
> 已发行未上市的新债**不进主批量池**（主池报告里记为「已发行未上市」）：它们还买不到，也没有市价可比，放进主池只会让偏差列一片空白。它们的去处就是关注池。
...
对称处理会把唯一的假设来源删掉——实测唯一一只机会分 ≥8 的债正是被"异常"标签踢出候选的。
