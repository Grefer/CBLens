...
回放"按规则选债 + 定期调仓"的历史表现, 并与**等权基准**对比。这是研究诊断工具,
不是收益承诺。

**三步上手** (首屏标题栏也印着这三步; 没结果时图表区有 👋 引导卡)

1. 「策略方案」选一个预设 (稳健打底 / 低估轮动 / 折价套利) —— 模板会一键设好
   **全部**参数 (含选券权重、现金收益、仓位择时), 不残留上次手动值; 日期默认近一年;
2. 确认「开始 / 结束」日期与「频率」 → 点「运行策略」(会自动预检)。历史口径默认「标准」(分钟级);
3. 只看三个数: **总收益 vs 等权基准** (净值图虚线, 跑赢没有)、**最大回撤**
   (最深亏多少)、**结论卡** (一句话定性)。

**参数分层 (该动哪些?)** —— 首屏只露上手必需项, 研究旋钮收在「高级设置」

| 区域 | 参数 | 普通用户建议 |
| --- | --- | --- |
| 首屏 · 基础 | 策略方案 / 开始·结束 / 频率 | 选预设 + 默认即可, 模板一键设好其余参数 |
| 首屏 · 选债 | 选债规则 · Top N · 基准 | 模板已设好; 勾「基准」叠加等权全池 + 中证转债指数两条对照线 |
| 高级 · 策略调参 | 选券权重 · 现金收益 · 仓位择时 · 成本 | **新人可不动** (均有默认且随模板归位)。选券权重两档均无稳健选股 alpha (见 README"模型边界"); 成本默认 20bps、现金收益 2.2% |
| 高级 · 回测数据 | 回测范围 / 历史口径 | 深入研究再动 |
| 高级 · 选债条件 | 价格 / 溢价 / 偏差 / HV 区间 | 深入研究再动 |

**历史口径怎么选**

- **标准**: 本地条款 + 补丁 + 事件回放, 分钟级; 历史发生过下修的债可能有转股价跳点偏差。
- **Wind高保真**: 逐债逐期从 Wind 拉历史条款, 最准但大池可能数小时;
  等价 CLI (`cb-strategy-backtest`) 配 `--cache-dir` 后复跑可大幅提速。

**结果怎么读 (四条铁律)**

1. 一切对照基准: 总览同时给「等权基准」(全可投池) 与「中证转债指数」两条线,
   跑不赢 = 这套规则没有超额, 哪怕绝对收益为正;
2. **先看「稳健性」卡**: 块自助给 Sharpe 置信区间与跑赢基准概率 —— **CI 含 0 = 差异
...
- **仓位择时**: 「估值缩放」按全市场中位偏差调总仓位 (贵→降仓), 是模型唯一跨牛熊·
  跨频率稳健的优势 (风险预算工具, 非收益增强); 默认「恒定满仓」。
- **现金收益**: 留现金/缺价/降仓留出的现金按此年化计息 (默认 2.2%≈货基); 设 0 复现旧口径。
  注意 Sharpe 课征无风险门槛, 现金 0 计息会系统性低估持现金策略。

CLI 等价命令: `cb-strategy-backtest --start 2025-01-01 --end 2026-01-01 --freq M
--holding-mode top_score --cash-yield 0.022 --cache-dir .cache/bt` (全部参数见 `--help`)。
