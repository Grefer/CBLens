...
可在 CBLens 中直接选择 `D:\Software\Wind\x64\WindPy.py`；当前版本的自动查找也会读取
独立 Python 安装目录中的这类记录。
...
| **Tab 切换** | ⭐ 关注 · 📦 批量 · ⚡ 定价 · 📈 回测 · 🔥 敏感性（启动默认落在 **⭐ 关注**） |
| **行情源** | 选择 Wind 或 akshare —— **全局唯一，各页共用**：关注池主页、批量定价、单债定价与回测走的都是这一个（各页不再各摆一个下拉）。默认值按本机接口文件是否可发现挑选；加载和连接结果可在「Wind 接口设置」中检测 |
...
![CBLens 2.0.0 批量定价页](../assets/cblens-batch-screenshot.png)
...
- **估值偏差**：默认要求比当期市场中位便宜至少 5 个百分点，再按 `(market - theoretical) / theoretical` 从低到高排序。市价可以高于理论价；只有显式设置「绝对偏差上限 = 0」才限制为市价不高于理论价。无法形成市场锚的小池不应用相对门槛。
...
| 参数设置 · 回测代码池 | 本地全市场 / 跟随批量页筛选 / 自选代码 | 本地全市场使用点时动态池；跟随模式显示批量视图名与名单数量 |
| 参数设置 · 选债限制 | 价格 / 溢价 / 估值偏差 / HV | 这些是准入范围，不参与排序 |
| 参数设置 · 模型与交易 | `p_down` / HV窗口 / 利差 / 困境斜率 / 股息率 / 成本 / 现金 / 事件退出 | 固定股息率是情景假设；全部参数随快照保存 |

基准固定自动计算，候选不足固定保留现金；点击运行时系统会先自动预检。CSV 导出和清空快照位于结果区右上角。

选择「跟随批量页筛选」后，代码池一行会显示“批量页「视图名」N只 → 每期按下方限制选债”，点击「去批量页调整」可修改来源视图。批量页切换视图或更新结果后，策略页的名单、数量及已有预检同步更新。下方限制是在这份名单内逐期选债，不会反向改写批量页的筛选规则。运行中的回测固定使用启动名单，后续变化标为「下次回测」；旧结果与冻结方案保留实际执行的名单。旧预设中的「当前筛选结果」会显示为新名称。
...
akshare 的股息率取数可能回落到实时快照，历史回测不能因此声称所有输入都已按时点重建。「诊断」单独展示股息率来源；已结束估值日的历史值进入持久缓存，实时值当日最多缓存一小时，失败缓存五分钟，真实的零股息率与取数失败分开记录。固定假设的对比实验可在 GUI 填「股息率%」，或在 CLI 传 `--q 0`（小数制，表示假设不计股息）；这不代表真实历史股息率为 0。

**资金与收益怎么记**

策略与等权基准使用同一份逐笔账本。信号后下一可得收盘执行；成交前旧持仓继续承担涨跌，缺价的退出指令保留到实际可成交日，不按旧收盘价虚构卖出。期末留仓按最近报价估值，界面明确区分「持有」「待成交」与真实退出。单边成本按实际买入金额与卖出金额分别扣除，成员不变时的等权再平衡同样计费；现金按实际可用金额逐日计息。

合同票息在日期与金额可确定时记为税前应收，确认支付后转成可用现金。强赎登记日、含混的现金流元组或名义到期日均不足以证明本金到账，不会据此释放资金。「持仓」的资金流水与 CSV 的 `ledger_entries` 可逐笔核对；「诊断」列出未确认支付、陈旧估值等限制。尚未提供确认支付数据的应收不等于可投资现金，也不代表完整税后总回报。

**在「对比」页验证方案**

1. 完成一次当前版本回测后，点击「稳健性对比」：最多七组单因素变体，比较持仓数、便宜度门槛和成本的邻近值。各组复用定价面板；结果给出收益、回撤、换手、持仓重合度及最优变体的多重检验校正。重合度不把双方全现金视为有效持仓样本。
2. 点击「选债 / 仓位对照」：同候选池等权、PDE Top N、PDE 加估值缩放三组保持成交、费用、现金与候选条件一致。第一组仅作研究对照，不改变正常策略的持仓选项。双击一行可查看该组完整结果。
3. 点击「冻结当前方案」：保存当前**结果实际执行**的配置、代码池、运行开始捕获的代码与数据指纹，以及训练结果。此时表单里尚未运行的修改不参与冻结；新方案独立保存，可形成版本链。
4. 在方案列表选择档案，把上方「结束」改为过去的完整调仓边界，再点击「更新样本外」。固定起点为冻结后第一个正常调仓日；不足一个完整区间时不取数。每次从同一起点完整重跑，核对全部已记录区间后只追加新期。经济口径或已记录结果变化会报冲突，不覆盖旧结果。实时股息率、当前条款回退等不满足历史时点要求的输入不能作为严格样本外记录。

研究报告保存在 `strategy_research_results/`，冻结档案与逐期样本外记录保存在 `strategy_experiments/`。后者独立于最近八份普通快照的自动清理，也不受「清空对比记录」影响。完整运行设置缺失的旧快照需要重跑后才能冻结。冻结的是规则与代码池，不能把反复调参后的历史结果重新标成未见过的未来样本。
