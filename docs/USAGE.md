...
**开发版桌面端（下述单按钮界面尚未包含在 RC6 包中）**：

1. 登录 Wind 终端，打开 **同步池 → Wind 接口设置**。
2. 路径留空，点击 **检测并连接**。应用优先查找本机 Wind 安装目录，再定位其中的接口并测试连接。
3. 未找到时点击 **选择文件…**，选择实际安装目录里的 `WindPy.py`，再点同一个 **检测并连接**。例如 Windows 为 `D:\Software\Wind\x64\WindPy.py`，macOS 为 `/Applications/Wind API.app/Contents/python/WindPy.py`，以实际位置为准。有路径时会严格检测该路径。
4. 接口可加载后点 **保存配置**，重启 CBLens。即使终端尚未登录，只要接口已加载，也能保存有效路径。

检测由用户主动发起，在独立进程中依次查找、加载接口和连接终端。总超时默认 45 秒，关闭设置窗口会取消检测。启动应用只查找接口文件，首次 Wind 取数需主动刷新。修改路径后旧结果会清除，避免把上一次的错误误认为本次结果。

Windows 优先从 Wind 安装提示（`WIND_HOME` / `WINDDIR`）和系统安装记录定位根目录，再查找固定的接口子目录；常见 Wind 路径以及独立 Python 的 `WindPy.pth` 作为补充，不扫描整个磁盘、不执行其他 Python。

RC6 仍是三个按钮：**自动检测**忽略输入框路径，**检测接口**使用输入路径加载模块，**测试连接**再验证终端。RC6 手动选好文件后需点后两个按钮之一。
...
Wind 修复工具的「配置成功」对应它列出的那套 Python，桌面包使用独立的运行环境。
例如该 Python 的 `Lib\site-packages\WindPy.pth` 写着 `D:\Software\Wind\x64`，
可在 CBLens 中直接选择 `D:\Software\Wind\x64\WindPy.py`。RC6 自动查找尚未读取
独立 Python 安装目录中的这类记录，后续修复版会自动跟随这些路径。
...
- Windows：GitHub Actions 的 `build-desktop.yml` 按 tag 检出、运行测试、构建，再通过 `scripts/check_windows_desktop.py` 以无控制台、无标准句柄的方式启动 EXE，检查首启种子、输出流补齐和离线进度条后才上传 Windows 包；手动触发时必须填写 `release_tag`。诊断 JSON 随 Actions 构建产物保留。
