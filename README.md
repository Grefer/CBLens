...
> [!NOTE]
> **🎯 策略页（组合回测）尚未完工，2.0.0 不在标签栏中开放。** 页面代码与命令行 `cb-strategy-backtest` 都原样保留，开发时可用环境变量 `CBLENS_ENABLE_STRATEGY_TAB=1` 打开；界面上的口径还在调整，不要拿它下结论。
...
- **定价页**：单债钻取 + 隐含波动率反解
- **回测页**：历史模型偏差复盘
- **策略页**：组合回测 —— **未完工，2.0.0 暂不开放**（命令行 `cb-strategy-backtest` 仍可用）
- **敏感性页**：σ-S 热力图 + 报告导出
