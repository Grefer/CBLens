...
# ⑧ 错定价策略回测 (默认稳健下修优势; --cache-dir 复跑提速)
cb-strategy-backtest --start 2025-01-01 --end 2026-01-01 --freq M \
  --rank-signal down_reset_robust_edge --cash-yield 0.022
```
...
- **「下修优势」视图会合法地变空**：稳健下修优势 = σ ±15% / 信用利差 ±100bp 四角点中最差的 (理论价 − 市价)，零点表示"即便按最不利的 nuisance 参数，模型价仍高于市价"。在贵市场里为正的标的天然稀少（2026-08 主池实测 14/280 = 5%），视图变空是信号在说话而不是判据坏了。该列由批量定价自动计算（每债约 +166 ms，全池按 10 线程折算 +4.6 s）。
...
- **错定价不等于已证实 alpha**：下修策略比较模型给定与市场隐含的下修强度，并在 HV/信用利差扰动下取最差优势；估值策略按市价相对理论价偏差排序。这些信号更贴近模型能力边界，但仍需样本外检验、参数邻域测试和基准对照；“稳健优势”只表示对当前扰动带不敏感。
