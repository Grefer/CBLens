...
- **机会分是复核标记、不是收益预测**：`opportunity_score` 经全市场池回测，其**横截面排序对未来收益的预测力≈0**（Rank-IC≈−0.05），因此新策略页不再用它做买入排名；旧快照/Python API 仅做兼容读取。更进一步，实测该分数的低估项在**92% 的债上恒为 0**（全市场中位偏差长期为正），分数实际由评级/余额加分与风险惩罚决定 —— 它在九成的债上度量的是信用质量而非错定价。批量页因此已改按**相对偏差**排序与筛选，`质量分`列把那部分单独摊开；机会分数值本身保持冻结，供 `rank_signal="score"` 与旧快照复现。
