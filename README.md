...
支持多数据源接入、公告事件解析、公开交易主池筛选、GUI 日常复核与 CLI 研究工具。<br/>
...
> [!NOTE]
> **2.0.0 暂不包含组合策略回测 GUI。** 命令行 `cb-strategy-backtest` 仍可使用；界面入口会在后续版本继续优化后再开放。

从 v1 升级涉及 API、CLI、CSV 和模型口径变化，请先读 [版本说明](CHANGELOG.md) 与 [v1 → v2 升级指南](docs/UPGRADING_V2.md)。

---
![CBLens 2.0.0 关注池主页](assets/cblens-screenshot.png)
...
### 🖥️ 研究界面
CustomTkinter GUI 覆盖日常定价与复核主流程：
...
组合策略研究当前使用命令行 `cb-strategy-backtest`；GUI 入口留待后续版本。
...
- **强赎路径依赖**：强赎触发是单点判断，尚未完整建模"30 个交易日中 15 日"的路径依赖；下修触发同理用单点 S 依赖强度近似。三处单点触发中，**回售已于 2026-09-02 改为"回售期内处处给底"**：原先按当前 S 掩码给底（回售价是常数，只加在低价侧）会让价值曲面在触发线上出现台阶，回溯把台阶抹成一段**非单调**凹陷，`dV/dS < 0` —— 可转债不可能有负 Δ。实测这不是离散伪影（M 从 300 加密到 4800，凹陷稳定在 2.08~2.14 元），全池 7 只有负 Δ 区域，逐条消融 100% 归因于回售（关掉强赎 7/7 逐位不变）。强赎那一侧早就有解药且是承重的（cap = `max(call_price, parity·(1+σ√t_grace))`，目标随 S 上升所以单调；把 `call_notice_days` 设成 0 会当场在 1.30K 长出负区）；下修是同一形状但要 `p_down = 2.0/年`（生产 0.25 的 8 倍）才咬合。**代价要认**：新口径比真实条款宽——真实是"**连续三十个交易日**低于 70%"外加"每年行使一次"的年度用尽规则，两者在数据模型里都没有字段（只有 `put_trigger_pct` 幅度与 `put_obs_months`），真做路径依赖要加一维网格（实测 31 倍运行时）且没有本地日线可以种计数器。实测影响：全池 298/311 只价格变动 < 0.005 元，最大 +0.45 元，**五个视图成员逐只相同、分桶 0 处变化、便宜度排名一名未动**。
...
- **历史股息率**：定价结果区分历史取值、实时快照、固定假设和失败回退。已增加跨运行缓存：已结束日期的历史值持久保存，实时快照最多复用一小时，失败最多记五分钟；缓存不改变数据来源。akshare 实时兜底仍不能代表历史信息，可显式指定固定 `q` 做可重复的情景研究，但这不等于恢复真实历史股息率；详见 [升级指南](docs/UPGRADING_V2.md#模型口径与旧研究结果)。
- **Gamma 边界**：下修价下限在一次求解中按初始股价冻结，附近的折点会让部分标的的 Γ 为负或随网格加密明显漂移。当前版本保留这一近似，Γ 不宜直接作为对冲或精细风险限额依据；价格结果也应结合下修情景复核。
- **历史回测**：策略回测推荐 `Wind高保真` 口径（CLI `--history-mode wind-high-fidelity`），按估值日查询历史条款并用公告重建状态；`标准` 口径适合快速诊断，正式结论需要高保真复核。GUI 策略页 2.0.0 未开放，这一路目前只走命令行。
...
- **机会分已删除**：`opportunity_score` 经全市场池回测，其**横截面排序对未来收益的预测力≈0**（Rank-IC≈−0.05）。更进一步，实测该分数的低估项在 **95% 的债上恒为 0**（全市场中位偏差长期为正），分数实际由评级/余额加分与风险惩罚决定 —— 它度量的是信用质量而非错定价（`Spearman(机会分, 质量分) = +0.517`，而 `Spearman(机会分, 偏差)` 只有 −0.640）。**该字段与其全部消费者（列、`min_score` 门槛、`rank_signal="score"`、CSV 列）已于 2026-08-29 整体删除**；`质量分` 保留，它本来就是从中拆出来单列的那一支。旧快照里的 `rank_signal="score"` 会落到「估值偏差」。
- **候选筛选锚当期横截面，不用绝对阈值**：模型理论价相对市价的中位偏差 4 年间在 +0.4% ↔ +21.6% 摆动，而"便宜尾"的形状（p25 − 中位）摆幅只有 4.2pp。因此「低估候选」判据 = 比全市场中位便宜 ≥5pp **且**排进当期最便宜的 15%（两道闸缺一不可：只有下限能表达"今天真没便宜货"，只有上限能挡住谷底时的几百只）。旧的 `机会分 ≥ 8` 绝对阈值在 2026-08 的主池上只剩 1/280 只候选。
- **错定价不等于已证实 alpha**：估值策略按市价相对理论价偏差排序，更贴近模型能力边界，但仍需样本外检验、参数邻域测试和基准对照。
- **理论价 vs 市价的系统性溢价是时变的**：模型对全市场的中位低估在 0%（2024-09 熊市谷底）到 +21%（牛市高位）间摆动、长期中枢约 +13%（4 年季度复检）。该聚合中位偏差可作转债**大类估值/择时**参考（`cb-valuation`、批量页估值横幅），与中证转债指数下一季收益历史负相关≈−0.52；但**个券绝对低估度不宜直接当便宜**。
