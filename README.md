...
- **机会分是复核标记、不是收益预测**：`opportunity_score` 衡量"模型低估程度"，用于筛出值得人工复核的标的。经全市场池回测，其**横截面排序对未来收益的预测力≈0**（Rank-IC≈−0.05，2025–2026 样本），**不构成选股 / 买入排名**。策略页"按机会分选 Top N"是研究诊断，须对照等权基准解读。可用 `signal_eval` 模块（Rank-IC / 分位 / 多因子）自行复验。
