...
散落在下文各处的「单独立项 / 尚未处理 / 已知边界」在这里汇总一次 —— 它们**没有**
任何 issue、任务或测试在跟, 唯一的记录就是文档本身, 所以至少要有一个地方数得清。
每条都是**测量过之后决定不改**的, 不是漏掉的; 动它们之前先读原处那一段的理由。
...
- **同一段口径不许有第二份实现 —— 单债回测页曾是 `price_from_provider` 的手抄副本**。
  `backtest._build_backtest_pricer_kwargs` 逐行抄自定价链路, 然后各自演化, 抄漏了四处;
  表现是同一只债同一天两个页面给出不同的理论价, 而页面上没有任何线索说得出为什么
  (与「视图归属两边各写一份」「事件短标签 GUI 自带一份」同源)。现已收口到
  `pricing_api.build_pricer_kwargs`, 两页共用, 抽取时逐位验过 (365 只在市债,
  363 只逐位相同 / 2 只两边同样抛错 / 0 差异)。四处分叉各自的量级:
  ① **「逐点读取条款」曾只是注释里的承诺**。函数开头写着"具体定价条款在每个采样日逐点
     读取, 避免把当前条款带回历史日期", 而 GUI 传进来的 `CachedBondDataProvider` 的
     `get_bond_terms(code, val_date)` **根本不看 val_date**。于是每个历史采样日都用
     **下修之后**的 K 定价 —— 而这一页的全部用途就是看模型偏差随时间怎么走。实测 12 个
     月度采样点: 全库 **322/1059 只 (30.4%)** 至少有一个采样日 K 用错, 采样点口径
     **21.4%**, 最大偏离 **115%** (128119.SZ 4.20 vs 当日 1.95); 端到端 127046.SZ
     2025-09 理论价 **134.64 → 108.47**。策略页早就在外面包了 `HistoricalBondDataProvider`,
     单债页漏了。**注释不是防线** —— 这条承诺在代码里一天都没成立过。
     现默认包这一层 (`point_in_time=True`), 传进来已经是那一层就原样用。
     **连带**: `_terms_source_diagnostic` 的 `uses_current_fallback` 此前是写死的
     `False` —— 页面在"没有逐日条款"时反而声称自己是逐日的。包了层之后它有真值可报。
  ② 已公告强赎 (`call_redemption_date`) 整个分支不存在: 到期日不截断、赎回价不替换、
     触发式 cap 不关。全库 **541 只**带这个字段。
  ③ `put_trigger_ratio` 缺值不显式关掉 → 落到 pricer 默认 0.7, 给 **69 只**本来没有
     回售条款的债凭空造一个回售权 (这个坑 pricing_api 修过, 副本没跟上 —— 副本的
     典型失败形态不是"抄错", 是"修的时候只修了一份")。
  ④ 评级信用利差下限没套。实测全库 **532/1059** 只的评级下限高于回测页默认的 0.03
     (A- 是 0.08, 2.7 倍) —— 于是"回测说模型准"与"批量页说模型贵"可以同时成立。
  **已知代价, 未处理**: 投影层会用公告解析的评级盖掉 cb_data 里 `cb-sync-ratings` 同步的
  第三方当前值 (实测 110081.SH 2026-08-31: BBB+ → A + 展望「负面」, 利差下限 0.10 → 0.06)。
  而体检实测 17 条分歧里 **15 条是公告 patch 错**。这是投影层的既有属性 (策略页一直如此),
  不是这次引入的, 但现在多了一个消费者 —— 要单独立项。
...
- **整段窗口预筛只许吃"随日期单调"的那两条 (2026-09-08)**。默认标的池是整个条款库,
  而 `_eligible_codes_for_date` 每期从头扫 —— 实测 1060 只里有一大批**每一期**都被同一个
  结构性理由剔除。`_codes_reachable_in_window` 在循环外只判一次:
  **到期日 ≤ 首个调仓日** (到期只会越来越早地成立) / **上市日 > 最后一个调仓日**
  (上市只会越来越晚地不成立)。实测池子 1060 → 838 (2025 月频, 摘 162 已到期 + 60 尚未上市)
  / 866 (2024 月频) / 975 (2022~2026 季频) / 756 (2026 周频)。
  **四个区间 79 期逐期比对 eligible, 不一致 0 期。**
  三条约定:
  ① **不许用「首末两期都落选就摘」这条看着很自然的规则**。反例: 一只债在窗口开始**之后**
     上市、在窗口结束**之前**到期 —— 首期「尚未上市」、末期「已到期」, 两端都落选而中间
     照常能买。而且这种错**不会红**: 回测照常跑完, 只是那只债从此不在池子里。守护测试
     的 fixture 里专门有这么一只 (110004.SH), 两个方向的变异都当场变红。
  ② **其余剔除原因一概不碰**, 哪怕看着也很结构性 (代码段/定向/非沪深)。要吃它们就得判
     "这个原因随不随日期单调", 而那个分类今天不存在 —— 现造一个 (按理由字符串前缀匹配)
     就是在选债路径旁边再立一份判据, 而 `_select_candidate_rows` 的 docstring 记着上次
     这么干的下场。两个方向的代价还**极不对称**: 少摘几只只是慢一点, 多摘一只是静默丢
     标的。停牌类原因尤其不能进 —— 它随日期反复横跳。
  ③ **条款取在 `schedule[0]`**, 那是第一期的估值日, 于是这一轮取数被
     `_BacktestCacheProvider` 原样复用, 预筛**不额外发请求**。取不到条款就保留 (与
     `_dynamic_pool_for_date` 的"无法获取条款, 保守保留"同口径)。
  **影响面比预想的窄, 是量出来的不是读出来的**: 首期实测 eligible 506 → 506 (相同)、
  `source_diagnostics` 791 → 791 (**相同**, 所以 `data_quality.sample_count` /
  `current_fallback_ratio` 一个字节不变 —— 那两个字段是在日期检查**之后**才 append 的),
  只有 `excluded` 从 554 → 332, 少掉的正好是那 222 条结构性理由。
  被摘掉的债不再进逐期 `excluded`, 所以在 `diagnostics["performance"]` 里按原因记一笔
  (`window_prefiltered.*`) —— 否则就是静默消失, 而"没有落选解释"和"根本没有落选"长得
  一模一样。
...
- **投影层的 `close` 改用一次宽窗口取数 (2026-09-08)**。
  ...
  一个窄窗口、且直接问 `self.inner` —— 于是**同时绕开**外层 `_BacktestCacheProvider`
  (那层按宽窗口存整段再切片) 与 `DiskCacheProvider` 的跨运行复用 (那层的键是精确
  `(code, 起, 止)`, 每个估值日都是一个新键, 冷跑一发不少)。实测单债 12 期: 定价路径
  1 发 + 11 次命中, **条款路径 12 发全落网络**; Wind 高保真下它还被全局 `_wind_lock`
  串行化, akshare 侧每发要过一次 MiniRacer 解密。
  改成 `set_close_window_hint(start, end)` + 实例内按 code 缓存宽窗口再切片。
  **真实数据逐日验过**: 拿磁盘缓存里 470 只债的真实日线 × 33 个估值日 = **15510 组
  比对, 差异 0**, 取数发数 15510 → 470 (**降 97.0%**)。
  五条约定:
  ① **15 天下限必须在切片时再套一遍**, 它是**判据的一部分**而不只是取数窗口: 窗口里
     一根都没有 → `close` 为 None (停牌/长假), 而不是退回更早的一根陈旧价。实测去掉
     它, 同一批真实数据上有 **67 组**从 None 变成十几天前的价 (110047.SH 连着三个估值日
     变成 117.805) —— 那是口径变更不是提速。收在 `_latest_close_from`, 窄宽两条路共用。
  ② **缓存序列, 不缓存 close 本身**: close 必须逐估值日重算, 缓存那个数就等于把上面
     那个坑原样重开。
  ③ **hint 由编排层按真实日程给** (`_hint_close_window`), 不在四个构造点各传一次 ——
     只有编排层知道频率与首末残期, 而窗口开小一格**不报错**, 只是那些估值日静默退回
     窄窗口 (提速没了而测试全绿)。终点夹到 `今天−1` (`DiskCacheProvider` 只缓存严格
     过去的区间), 落在窗口外的估值日原路走窄窗口 —— GUI 默认区间的最后一期可能就是今天。
  ④ **空序列不记住**, 与 `_BacktestCacheProvider._remember` 同一条理由; 且不再回落窄
     窗口 —— 窄是宽的子集, 宽的取不到窄的也取不到, 再打一发只是把失败重复一次。
  ⑤ **不走"加 `market=` 参数让内外层共用一个 DiskCache 目录"那条路**: (a) 那是第二条
     取价通道, 接错源时准入用的 close 与其余环节来自不同数据源且不报错 (`_build_backtest_pricer_kwargs`
     手抄那次事故的形状); (b) DiskCache 的键是精确区间, 13780 个窄窗口就是 13780 个键,
     冷跑照样一发不少; (c) 两个 `_identity` 不同的 DiskCacheProvider 共用一个目录会互相
     判"身份变更 → 弃用旧缓存", 每跑一次清一次盘, 只留一条 `logger.info`。
  **单债回测页不设 hint**, 那条路径行为逐字不变。
...
- **区间事件的 `effective_end` 要逐类型验过才能用**: 实测全库 7794 条,
  ...
  **2026-09-02 补: `start` 也一样脏, 而且更常见**。实测 508 条 conversion_suspension 里
  **155 条 (31%)** 的 `effective_start` 早于公告日 30 天以上、141 条早于 180 天、最远
  **2072 天** (128136.SZ 立讯转债 2026-07-07 公告 → start 2020-11-03) —— 裸的
  `(?:自|从)` 锚抓到了正文里对转股期/回售期的引述。所以**不要以为 start 比 end 可信**。
  现按方向性不变量拦: 公告总是**提前**发 (提前量中位 5 天、最长 13 天), 起始日早于
  公告日 30 天以上就丢掉 `start` —— **只丢 start**, end 走另一套锚点, 两者可信度互不担保。
  同族还有一条: 承诺期 (`down_reset_rejected` / `call_no_redemption`) 的 `end` 不可能
  早于公告日 (正文成段引用上一期承诺, 解析器抓第一段), 实测 28 条是这个形状 —— 天能转债
  四份公告 (2025-08 / 2025-12 / 2026-06 / 2026-08) 全部解析成同一个 2024 年窗口, 于是
  一份**还在生效**的不下修承诺被当成已过期, 下修价值照常计入。
  **存量不会自动变**: 这两条只作用于新解析的事件, 库里 155 + 28 条要跑 `cb-repair-events`
  才回洗。
- **公告没取完不许推同步水位**。`cninfo` 的翻页此前有三种**静默**截断: 第一页之后任何
  一页失败 / 响应缺 `totalAnnouncement` (默认 0 让 `page*size >= 0` 立刻为真, 一页就停) /
  `max_pages × page_size` 用尽而总数更多 (实测 600 条公告只取回 300, 零日志)。而
  `sync_cb_events` 对三种一视同仁: 不记 `failed`、照常 `mark_synced` —— **把水位推过它
  从没看见的公告**, 那些公告之后永远不会再被拉取。现在走 `IncompleteAnnouncementList`
...
- **cninfo 也按出口 IP 限流, 但它表现成*读超时*, 而且全库同步的请求量是自找的
  ...
  **已修的是止血那两条**: `_retry` 加 `exhausted_is_rejection=` —— 判据放在"**重试打光了
  ...
  18.9 小时 → 约 1 分钟。
- **全库公告同步按*回看窗口*缩范围 (2026-09-08)**。全库一轮的请求量本身就是 cninfo
  限流的触发器 (上一条), 而**在窗口开始前就已经离场**的债在这个窗口里不可能有新公告 ——
  实测回看 180 天时全库 1060 只里 **623 只**是这一档, 约占请求量 **72%** (剩 437 只)。
  判据**与回看窗口绑定**而不是"是不是死债": `--lookback-days` 放大时跳过的自然变少
  (365 天 → 519 只), 不必再加一个开关去描述同一件事。
  **`market_exit_date` 只认既成事实 (摘牌日 / 最后交易日), `maturity_date` 是排期不是
  事实** —— 强赎债提前退市, 摘牌日早已过去而名义到期日还在未来, 实测全库 **222 只**
  是这个形状 (110080.SH 摘牌 2023-12-06 / 到期 2027-04-12)。中间版本按"所有终止日取
  最晚"判, 把这 222 只全部当成在市: 缩范围恰好**在最该起作用的那一批上失效**, 而且
  一声不响 (实测可跳数 623 → 333)。只有一个既成事实都没有时才退回 `maturity_date`,
  那一档是自然到期。有守护测试直接钉这个形状。
  保守方向与准入层一致 (**字段明确才剔除**): 读不出条款 / 一个日期都没有 / 离场日恰好
  等于窗口起点, 一律**保留** —— 跳错一只是静默丢公告 (水位那头不会有任何提示), 多留
  一只只是多花几次请求。**显式 `--codes` 不过滤**: 那是用户点名要的几只, 已离场也照同步;
  `--all` 关掉缩范围。
...
- **S0 以下的格点数有下限 (`_MIN_NODES_BELOW_S0 = 60`), 不够就自动加密**。网格在
  `[0, S_max]` 上均匀, 而 `S_max = min(exp(3σ√T), 50)·K` **只由 K 与 σ√T 定, 与 S0 无关**
  —— 高 σ / 深度 OTM 的债于是把分辨率全花在一段没有价值的高价区。实测生产 M=500 下
  主池 311 只里 **50 只**在 S0 以下不足 20 个格点, 最少的 118066.SH 只有 **5.6 个**;
  价格误差最大 **+2.78 元 (+1.90%)**、vega **+33.6%**, 而 N 早已收敛 (2000→8000 中位
  只动 0.0013 元) —— 纯空间误差。
  **修法是加密不是缩小 `S_max`**: 后者会截断定义域, 实测把 `_S_MAX_CAP` 从 50 降到 6
  最大误差反而从 2.72 涨到 **5.11 元**。加密单调改进, 没有新失效模式。
  60 这个下限是量出来的: 受影响的 73 只误差中位 0.0622 → **0.0035**、最大 2.72 → **0.77**,
  全池耗时 19s → 25s; 提到 80 只多换 0.001 的中位改善却多花 24% 时间。
  上限 `_MAX_ADAPTIVE_M = 4000` 防止 S0/S_max 极小时把 M 撑到天文数字。
  连带: 「低估候选」换了 1 只 (40 → 40, 一进一出), 分桶 0 处、便宜度排名一名未动。
- **⚠ 已知边界: 冻结的下修价下限在 ≈1.02·S0 处留了个折点, Γ(S0) 因此可能为负**。
  `pricing_api._estimate_down_reset_floor` 返回 `max(20日均价, 前一交易日收盘)` ——
  后者**就是 S0**, 所以 floor ≥ S0 恒成立, 实测主池 **184/311 只 floor 恰好等于 S0**。
  而 `_down_reset_value` 的有 floor 分支是 `target_k = max(S/premium, floor)`, 它在
  `S = premium·floor ≈ 1.02·S0` 处从常数切成线性 —— 折点正好落在读价格/Δ/Γ 的那一点上。
  实测 **39/311 只**的 Γ(S0) < 0 (最深 127062.SZ −0.75)。
  **根因是 floor 被冻结在今天**: 真实监管下限是"下修**当时**的 20 日均价", 应当随 S 走;
  随 S 走时 `target_k` 对 S 线性, 折点消失。
  **为什么没改**: 实测把 floor 改成按 S 比例走, 39 只负 Γ 全清零, 但价格中位 **+0.89 元**、
  均值 +1.91、最大 **+11.13 元**, 192/311 只动超过 0.5 元 —— 与下修价值本身同量级的
  **口径变更**, 不是补丁 (对照: 回售那次口径变更全池最大只动 0.45 元)。而且"随 S 走"用
  哪个比例本身是建模问题: 下跌行情里 20 日均价高于现价, ratio 该 >1, 而今天 184 只的
  ratio 恰好是 1.0 只因为它们最新收盘高于 20 日均价。要动这里请单独立项。
  **同一张面还让 Γ 对网格步长不收敛, 这是 Γ<0 的孪生症状**。实测全池 (Γ ≥ 1e-3 的
  242 只, M ∈ {500,1000,2000}, N=1000): 中位漂移 0.21%, 但 **11 只 >10%**, 最大
  111020.SH **77.3%** (Γ = 0.484 / 1.021 / 2.129)。四只最差的逐条消融, **`p_down=0`
  让漂移全部归零** (77.3/66.8/37.4/25.1% → 0.0%), 而去掉 floor 只减不消 —— 所以它是
  下修那张面的伪影, 不是网格分辨率不够。守护
  `test_gamma_stable_across_grid_refinement` 因此**只测夹点以上那一段**:
  它的 fixture 在 M=500/1000/2000 上有效 M 全被自适应夹成 3221, 三个 Γ **逐位相同**、
  相对极差恒等于 0.0 —— 那条 `drift < 0.10` 什么也观测不到 (旧版就是这么写的)。
  现在改测 M=3221/4000/8000 (实测 2.50%), 并显式断言"三个值互不相同", 免得它们
  又落进同一个有效网格。把生产那 11 只写进断言等于要求一个尚未做的口径变更。
  Γ 的两条守护**必须带 `down_reset_floor`** —— 不给就走无 floor 分支, 那是生产中不存在
  的形状 (有守护测试扫这两条用例的源码)。
- **希腊值的扰动解必须与基准解同网格** (`_price_grid(..., s_max_override=)`)。
  `S_max` 随 σ 变, 所以 vega 的 σ+1pp 那一解 h 会变、强赎触发掩码
  `S_grid >= K·call_trigger_ratio` 会吸附到不同格点 —— 而 vega·1pp 相对价格只有 1e-3
  量级, 两个各带网格误差的数相减不抵消反而放大。实测 vega 偏 **+33.6%** (万凯转债
  0.6190 vs 收敛值 0.4633), 锁网格后 **−2.1%**。
  **自适应加密没有替代它**: 加密之后抖动小了但没消失, 127053.SZ 实测锁 0.489 / 不锁
  0.338, 仍差 **30.8%**。Θ 已经是同一次回溯里截片, 天然共享网格。
- **Θ 的票息按*日历*计入, 不按"截片窗口罩没罩住它"**。截片窗口是 (今天, 今天+t_slice],
  而 `t_slice` 随 N 变 (N=1000 时 1.46 天, N=2000 时 0.73 天) —— 于是"明天付不付息"
  这件事由**网格步长**决定。实测同一只债在两套**生产**口径下 (批量 M=300/N=1000 vs
  单只 M=500/N=2000) 六个久期里 **3 个符号相反**, 量级差 ~250 倍 (−0.68 vs +0.005);
  N 扫描同样在 −0.45 / −0.91 / +0.005 之间跳。
  **上一版把这写成"已知局限: 只说得清方向, 说不清速率"—— 方向恰恰是说不清的那一半**,
  而那句话正是让它躲过复核的原因。处置: 先把窗口内票息加回扰动解得到纯衰减率, 再减去
  真正在**次日**支付的那笔。脏价口径不变 (票息照样进 Θ, 与 `price` 同底), 变的是它
  出现在正确的那一天; 改后 4y 那档 N ∈ {500…16000} 稳定在 **−0.9949**。
  **窗口天数必须与回溯里的 `step_date` 用同一把尺** (`int(t*365)` 截断, 不是 `round`)
  —— 用 round 会加回一笔解里根本没掉过的票息, Θ 照样翻号 (中间版本实测过)。
- **三处触发全是单点近似, 但只有强赎那处的形状是安全的**。真实条款都是路径依赖的
  (强赎「30 日中 15 日 ≥130%」/ 回售「**连续** 30 日 <70%」/ 下修同形), 而 pricer 只看
  当前 S。**决定曲面单调性的不是"单点近似"本身, 而是那个约束的目标随不随 S 走**:
  · 强赎 `min(V, max(call_price, parity·(1+σ√t_grace)))` —— 目标**随 S 上升**, 单调保住。
    这个 grace 是承重的: 把 `call_notice_days` 设 0, 嘉诚转债当场在 1.30K 处长出负 Δ 区
    (min dV/dS = −0.606)。
  · 回售 `max(V, 面值+应计)` —— 目标是**常数**, 只加在低价侧就是一个台阶。见下条。
  · 下修同形 (常数化的 reset_value 加在低价侧), 但要 `p_down = 2.0/年` (生产 0.25 的
    **8 倍**) 才有债转负; 而它的触发线**更拥挤** (90/311 只落在 S/K 0.75~0.95, 回售带
    只有 66 只) —— 同一个隐患的下一站。
- **回售期内处处给底, 不按当前 S 掩码** (2026-09-02 口径变更)。
  原实现 `V[S <= K·ratio] = max(V, put_price)` 让曲面在触发线上有台阶, 回溯把台阶抹成
  一段**非单调**凹陷, `dV/dS < 0` —— 可转债不可能有负 Δ。**不是离散伪影**: M 从 300
  加密到 4800 (h 缩小 16 倍) 凹陷稳定在 2.08~2.14 元 (常银转债), 负区 S/K 带稳定在
  0.629~0.783; N>1000 无关。全池 7/311 有负区, 逐条消融 **100% 归因于回售** (关掉回售
  7/7 消失, 关掉强赎 7/7 逐位不变, 关掉下修 5/7 更糟)。
  **今天没有人被误导** —— 311 只在实际 S0 上 0 只负 Δ。它被 `down_reset_floor` 遮着,
  那个估算一失败 (正股 60 天内不足 20 个收盘价) 山路转债立刻显示 −0.1441。
  **新口径比真实条款宽**: 真实是「连续 30 个交易日」外加「每年行使一次」的年度用尽,
  两者在数据模型里**都没有字段** (只有 `put_trigger_pct` 幅度与 `put_obs_months`)。
  所以这是"按当前 S 掩码"与"路径依赖"之间那个**结构上单调**的上界 ——
  `V = max(V_连续, 常数)` 且 `V_连续` 对 S 单调增 ⇒ Δ ≥ 0 是**不可能违反**的, 不是
  "实测没发现"。实测影响: 298/311 只价格变动 < 0.005 元, 最大 +0.4523 元 (燃23转债),
  **五个视图成员逐只相同、分桶 0 处、便宜度排名一名未动**。
  **另外四个方案为什么没选** (都实测过, 不是纸上比较):
  · **不改只记文档**: 代价 0, 论据也真 (今天 0 人被误导), 但缺陷恰好被另一个功能遮着,
    而遮罩会在数据变差时消失 —— 那正是最需要看清的时候。
  · **照搬强赎的 σ√t 软化**: **失败且是结构性的**。原型 `w = min(2Φ(−ln(S/B)/(σ√tg)), 1)`,
    V 连续了但 7 个负区还剩 **5** 个 (常银 min Δ = −2.46) —— 常数底乘一个随 S 衰减的
    权重, 本身就随 S 下降。任何"给常数底加权重"的软化都继承这个毛病。还要 +28% 运行时
    和一个新自由参数, 而那个参数几乎不影响结果 (15/30/60 天扫下来最大只差 0.03 元)。
  · **真做路径依赖**: 唯一正确, 但 **×31 运行时** (单只 62ms → 1906ms, 全池 19s → 约
    10 分钟单线程), 且 `BondTerms` 没有天数字段、本地没有日线可以种计数器。而它的全部
    回报被 A~D 那个区间封顶 —— D ≥ A 对 311/311 成立, 区间最宽 **1.00 元**。付 31 倍
    买 ≤1 元。
  · **只改展示**: `delta` 唯一的消费者是 `gui/controllers/pricing.py` 那一个展示格,
    而今天 311 只全为正。它唯一能遮住的就是 `down_reset_floor` 失败那条路。
  **两条当时测过、但没跟着决定一起记下来的账** (2026-09-03 复测):
  · **回售条款形态的普适性没有验过**。AGENTS 与 README 都说新口径是「连续三十个交易日」
    的结构性上界, 而"这批债都是这个条款形态"在库里**无从确认**: 库里有
    `put_trigger_pct` 的 990 只, 而 `data/announcement_text_cache` 的 1399 份正文里
    含「连续三十个交易日」的只有 **305 份**。剩下的债可能是别的形态, 上界论证对它们
    只是**假定**成立。要真验就得给条款形态建一个字段。
  · **强赎 cap 削掉的价值比回售那个伪影大得多, 而那是个没人判过的口径问题**。
    实测 311 只里 **65 只**因触发式 cap 少 0.005~**7.94 元** (118062.SH, σ=1.00,
    S/K=1.47; 消融法: 把 `call_no_redemption_until` 推到到期), 全是高 σ 名字
    (σ 区间 0.43~1.21)。对照: 回售那次口径变更整个经济内容被界定在 **1.00 元 / 1 只**,
    而它被处理了。两者的区别是**性质**不是量级 —— 回售那个是单调性缺陷 (Δ<0, 不可能),
    强赎这个是"cap 定得对不对"的取值问题, 没有一个像"Δ 必须 ≥0"那样的不变量能判它对错。
    所以它留着; 但**留着的理由要写出来**, 否则下一个人看到的只是"没人管过"。

  **标定用**: 回售底本身值 **最大 26.75 元** (宏图转债 88.79 → 62.04), 而整个残差
  ≤1.00 元 —— 这是"正确的大功能上的窄边角", 不是功能坏了。改它之前先记住这个比例。
- **`put_trigger_ratio=None` 表示"这只债没有回售条款"**, 与"有条款但比例未知"是两回事。
  `pricing_api` 曾只在 `terms.put_trigger_pct` 非空时才传参, 于是银行/券商转债 (全库
  69 只、主池 5 只: 上银/财通/兴业/重银/常银) 落到 pricer 默认 0.7, 被**凭空造出**一个
  回售权。今天影响 ≤0.0014 元 —— 但那是它们 S/K 都在 0.87 以上的巧合: 把常银转债的正股
  放到 0.60K, 虚构的底让它**虚增 5.614 元**。这条必须排在上一条**之前**修: 处处给底会
  把这 5 只的误差从 0.0014 元放大到整条曲面。
