...
def build(app, tab):
    ...
    _form_row(sec1, "波动率 σ (%)", app.v_sigma, 2, wind=True, width=80,
              source_var=app.v_src_sigma, show_source=True,
              tooltip="年化历史波动率。可在高级模型参数中切换估算窗口。")
    _form_row(sec1, "信用利差 (%)", app.v_spread, 3, width=80,
    ...
    ctk.CTkLabel(event_row, textvariable=app.v_dr_status, text_color=TEXT,
    ...
    _form_row(sec_terms, "各年票息 (%)", app.v_coupons, 6, wind=True, width=180,
              source_var=app.v_src_coupons, tooltip="逐年票息百分比, 逗号分隔。")
    ...
    _form_row(sec2, "无风险利率 r (%)", app.v_r, 1, width=80,
              extra_widget=make_shi, source_var=app.v_src_r, show_source=True,
              tooltip="无风险利率, 默认可用 Shibor 1Y 近似。")
    _form_row(sec2, "下修强度 p (%/年)", app.v_p_down, 2, source_var=app.v_src_p_down,
              tooltip="年化下修事件强度。公告不下修冻结期内会被事件表自动屏蔽。")
    _form_row(sec2, "信用扩张系数 (%)", app.v_dk, 3, source_var=app.v_src_dk,
