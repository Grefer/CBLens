...
class UniversalCBPricer:
    ...
    def down_reset_sensitivity(
        self,
        sigma: float,
        r: float,
        base_spread: float,
        p_down: float = 0.1,
        distress_k: float = 0.0,
        M: int = 300,
        N: int = 1000,
        bump: float = 0.01,
        q: float = 0.0,
    ) -> float:
        """年化下修强度增加 ``bump`` 时的理论价变化.

        默认 ``bump=0.01`` 表示 ``p_down`` 增加 1 个百分点, 因而返回值可直接
        解读为“理论价 / +1pp 下修强度”。该量用于把概率差转换成近似价格优势,
        不是概率本身。
        """
        if p_down < 0:
            raise ValueError("p_down must be non-negative")
        if bump <= 0:
    ...
    def solve_implied_p_down(
        ...
        """反解使理论价等于市场价的年化下修强度, 无解时返回 NaN.

        ``p_down`` 是事件强度而非一年内概率; 每步仍按
        ``1 - exp(-p_down * dt)`` 转为发生概率。目标价低于无下修价格、或高于
        ``p_down_hi`` 对应价格时没有可解释的隐含强度, 返回 NaN 而不夹到边界。
        """
        ...
        if (
            ...
            or target <= 0
            or p_down_lo < 0
            or p_down_hi <= p_down_lo
        ):
            return float("nan")

        def diff(intensity: float) -> float:
        ...
        try:
            ...
            return float("nan")
        if abs(f_lo) <= 1e-10:
            return float(p_down_lo)
        if abs(f_hi) <= 1e-10:
            return float(p_down_hi)
        if f_lo * f_hi > 0:
            return float("nan")
        try:
            return float(brentq(
