...
#   v3: 准入层收窄成"买不买得到" (2026-08-31)。评级过低 / 正股 ST 不再是硬剔除 (改由标签
#       标注 + 策略层阈值筛), 「已发行未上市」也进池 —— 主池 282 → 311。
#       同一批定价结果下的对照实测: 中位偏差 +21.30% → +20.55%, 下移 **0.75pp**
#       (新入池的多是低评级券, 系统性偏便宜)。
#
...
CALIBER_CHANGES: dict[str, dict[str, str]] = {
    ...
    CALIBER_V3: {
        "since": "2026-08-31",
        "summary": "准入层收窄成「买不买得到」— 评级/ST 不再硬剔除, 已发行未上市也进池",
        "impact": "主池 282 → 311; 同批对照实测中位偏差 +21.30% → +20.55%, 下移 0.75pp",
    },
}
