...
只会写不会撤, 删掉错事件并不能自动把它写坏的摘牌日改回来。具体动作::

    cb-sync-tradable --reset-status     # 把状态字段擦回数据源口径 (破坏性)
    cb-sync-events --apply              # 用修好的事件表重放
    cb-sync-admission-status            # 补回 Wind 侧的状态字段

**不要**指望不带开关的 ``cb-sync-tradable`` 顺手完成第一步: 它默认保留本地已有的状态
字段 (否则每次全量同步都会把状态刷新的成果整批清空, 实测让主池 284 只债的六条准入判据
全部失去输入)。``--reset-status`` 才是这里说的"恢复成 Wind 口径"。
