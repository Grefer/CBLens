...
# **兼容常量 —— 2026-08-31 起已无任何取值方**, 留着只为让旧策略快照读得回去。
#
# 它曾经是 ScoreStrategyConfig.exclude_risk_tags 的默认值, 那时"逐字冻结"是硬要求:
# 写成 tuple(sorted(HARD_REVIEW_TAGS)) 会让任何为改批量页展示而增删标签的动作自动变成
# **默认选债行为变更**。当时实测这个集合极敏感: 默认候选池 59 只; 改成只排"数据质量+
# 可交易性" → 262 只; 单去掉「偏差异常」→ 125 只; 去掉「模型溢价高」→ 94 只。
#
# 现在选债走 ScoreStrategyConfig 上的一组显式数值阈值 (`_candidate_filter_reason` 是唯一
# 的选债路径), `exclude_risk_tags` 的默认值是**空元组**, 这个集合在生产代码里没有取值方。
#
# ⚠️ 这里曾写着"「正股风险」留在集合里 ⇒ 本次改动对**策略结果零影响**, 两条路的结果
# 逐只相同"。**那句话现在是假的, 而且当时也只对了一半**:
#   · 它已不是 exclude_risk_tags 的默认值, ST 现在由 cfg.exclude_underlying_st 挡;
#   · 更要紧的是"零影响"从来只覆盖**逐行**判据。ST 债进池会改变**横截面**: 它们进
#     `median_deviation_of` 的锚, 也占 `cheapness_rank` 的名次, 而
#     `_select_candidate_rows` 是**先**跑 `filter_batch_results_by_view` (含「低估候选」
#     最便宜 15% 的名额上限) **再**逐行过阈值 —— 于是被随后剔掉的 ST 债照样吃掉了名额。
#     实测往 284 行截面注入 6 只合成 ST 困境债, 「低估候选」11 → 9, 挤掉两只非 ST。
# 这一条已登记在 AGENTS 的 deferred 表里 (改它是默认选债行为变更, 要单独立项)。
